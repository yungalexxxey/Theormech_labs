...
x = (1+sp.sin(5*t))*sp.cos(phi)
y = (1+sp.sin(5*t))*sp.sin(phi)
...
Rline,=ax1.plot([X[0],X[0]-Rho[0]*(-VY[0])/math.sqrt(math.pow(-VX[0],2)+math.pow(-VY[0],2))],[Y[0],Y[0]-Rho[0]*(-VX[0])/math.sqrt(np.power(-VX[0],2)+math.pow(-VY[0],2))],'b')
...
def anima(i):
    ...
    Rline.set_data([X[i],X[i]+Rho[i]*(-VY[i])/math.sqrt(math.pow(-VX[i],2)+math.pow(-VY[i],2))],[Y[i],Y[i]-Rho[i]*(-VX[i])/math.sqrt(math.pow(-VX[i],2)+math.pow(-VY[i],2))])
...
anim = FuncAnimation(fig, anima,frames=10000, interval=200, blit=True)
